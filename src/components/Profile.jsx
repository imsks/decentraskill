...
function Profile() {
    ...
    useEffect(() => {
        ...
        ;(async () => {
            try {
                let user
                if (navigate.location.pathname.includes("company")) {
                    user = await state.contract.methods.companies(id).call()
                    let res = 0
                    res += (
                        await state.contract.methods
                            .curr_emp_of_company(id)
                            .call()
                    ).length
                    res += (
                        await state.contract.methods
                            .prev_emp_of_company(id)
                            .call()
                    ).length
                    setNumEmp(res)
    ...
    }, [id, state.contract, navigate.location, setState])
        ...
        <div className='flex flex-row h-full w-full p-3 items-center justify-items-stretch text-gray-600'>
            <div className='w-4/6 h-full'>
                ...
                <div className='text-3xl'>
                    {navigate.location.pathname.includes("user") && role}
                </div>
                <div>
                    <h1>{location}</h1>
                </div>
                {navigate.location.pathname.includes("company") && (
